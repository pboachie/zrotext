...
|---|---|
| Q1–Q4 | Exact pin/manifest/transition bytes and browser→Android verification work in a test. A test-only browser IndexedDB adapter now exercises atomic local high-water across tabs. Independent owner comparison, accepted role/scope policy, lost-root ceremony, trusted clock, production rollback-resistant high-water, withheld-update and cross-client fork witness remain open. |
| Q5 | The dormant recipient provider now reconstructs exact outbound profile-02 `info` and enforces device role, with explicit empty HPKE AAD and a non-exportable API 31+ Keystore key. ZROtext owns the public-JCA HPKE composition; Tink's helper remains test-only. API 31 behavior, final provider/support acceptance, reboot/key-loss lifecycle and physical security level for draft 02 remain open. |
...
Tink is pinned only on the instrumentation-test classpath and is absent from the release runtime. The public-JCA HPKE candidate is compiled into the app as dormant code, while the signed-envelope receiver and manifest verifier remain instrumentation-only. No production path calls the candidate and no profile-02 route is enabled.

## Separate authorization candidate

The [Manifest02 authorization candidate](zt-sealed-draft-02-manifest-candidate.md) proposes the exact owner-signed manifest and dual-signed root-transition bytes, semantic **unsigned-byte** manifest digest, role/scope and freshness rules, and strict low-`s` signatures exercised by this test-only Android proof. The TypeScript verifier retains private, defensive snapshots of verified authority so mutating a returned `Uint8Array` cannot change its authorization result. Its test-only IndexedDB adapter commits local high-water with a compare-and-update transaction, but cannot detect storage deletion, restored backups, or a lying clock. Neither language's helper is a production trust store or stable provider contract. Q1–Q4 and Q6–Q11 stay open until the proposed choices are accepted and the remaining release evidence exists.
