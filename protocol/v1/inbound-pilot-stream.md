...
migration 007. It does not authorize an SMS send.
...
`classification` also parses `sim_unverified`, `send_unverified`, or
`encryption_unverified`, but the first Android uploader sends only
`captured_local`. The server assumes `metadata_only` content kind. The P-256
...
time; zero is valid. This frame is a metadata transport pilot, not a reviewed
