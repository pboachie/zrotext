...
| Q5 | Public JCA primitives pass an RFC 9180 known answer and an API 36 AVD cross-client envelope, but ZROtext owns the HPKE composition. Tink's helper remains in `hybrid.internal` and test-only. API 31 device behavior, provider/support decision, reboot/key-loss lifecycle and physical security level for draft 02 remain open. |
