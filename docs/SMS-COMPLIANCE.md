...
The pilot recognizes exact, case-insensitive STOP, STOPALL, UNSUBSCRIBE, CANCEL, END, QUIT, REVOKE and OPTOUT. Likely free-text withdrawals create a conservative block marked for review. Exact START or UNSTOP can clear only an existing suppression from a trusted reply in the same outbound-attempt reply window; this transition does not establish consent for a new message purpose. The phone does not send automatic confirmation SMS. A local block applies as soon as a recognized opt-out is captured, including while the relay is offline. Once its signed event reaches the writer, the account-scoped `recipient_suppressions` table is checked during acceptance under the same account lock used for transitions; exact retries are rejected while active.

A non-allowlisted route still requires authenticated capture beyond the existing outbound-attempt reply window, stable selected-line identity, a way to handle off-channel and ambiguous withdrawal requests, and end-to-end device evidence. The current pilot's signed inbound event proves the enrolled device and selected subscription in its local journal, but does not bind a durable line ID. Keep the general route closed until those gaps are resolved.
