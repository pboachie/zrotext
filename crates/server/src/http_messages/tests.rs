...
async fn postgres_alpha_http_accept_status_cancel_are_tenant_and_device_scoped() {
    ...
    let metered = router(
        MessagesHttpState::new(url.clone(), hasher.clone(), metered_policy.clone(), true).unwrap(),
    );
    ...
    client
            ...
            .unwrap();
    let billing_off =
        router(MessagesHttpState::new(url.clone(), hasher.clone(), metered_policy, false).unwrap());
    ...
    let active_app =
        router(MessagesHttpState::new(url.clone(), hasher.clone(), active_policy, true).unwrap());
    ...
    let race_app =
        router(MessagesHttpState::new(url.clone(), hasher.clone(), race_policy, false).unwrap());
